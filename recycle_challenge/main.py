...
if __name__ == '__main__':
    ...
    args.add_argument("--base_lr", type=float, default=0.001) #0.111 이하, 0.037...?
    ...
    model = ClsResNet(block=models.resnet.BasicBlock,
                        #김윤기's block repeat
                      layers=[3, 4, 6, 3],      #2, 2, 2, 2 -> ResNet18
    ...
    ## 수정사항 : 옵티마이져
    optimizer = AdamW([param for param in model.parameters() if param.requires_grad], lr=config.base_lr, weight_decay=1e-2) #0.0001

    ## 수정사항 : 스케쥴러
    #scheduler = StepLR(optimizer, step_size=18, gamma=0.9) # 0.9>gamma>0.5
